...
if (isset($_GET['cadastrar'])) {
        ...
        if (!empty($_GET['horario']) && !empty($_GET['dt']  )) {
            ...
            $sqlVerificaHorario = "SELECT COUNT(*) as total FROM ConsultaDentista WHERE horario = :horario";
            ...
            if ($totalConsultaDentista > 0) {
                ...
                // Inserir os dados na tabela ConsultaDentistas
...
if (isset($_GET['update'])) {
    ...
    ##codigo sql
    $sql = "UPDATE  ConsultaDentista SET horario= :horario, dt= :dt, funcionario= :funcionario, tipo= :tipo, aluno= :aluno WHERE id= :id ";
    ...
    if ($stmt->execute()) {
        // Redirecionamento para listaConsultaDentista.php com o valor do parâmetro 'dt'
        header("Location: lista-horarios-dentista.php?dia=" . $dt);
...
if (isset($_GET['remarcar'])) {
    ...
    try {
        ...
        header('Location: calendario-aluno-dentista.php');
        exit; 
    } catch (PDOException $e) {
        echo "Erro ao executar a consulta: " . $e->getMessage();
    }
